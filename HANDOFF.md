...
These are **stale comments/UI text describing removed behavior** — not bugs
in the actual rule logic, which is correct and fully tested. Flagging so a
future session doesn't get misled by them. **Application code was not
modified during this documentation pass** (per instructions), so these still
need a small cleanup pass:

1. `src/ui/rules.ts`, `PIECE_SUMMARY.flagBearer` (~line 120): still says
   "once inside (unladen) it may only move to G7 to collect the flag." This
   describes the **old** mandatory-collection rule that item 5 above
   replaced. The actual behavior (buffer/decision, free movement) is
   correctly described in the `RULES_SECTIONS` entry titled "Unladen
   Sanctuary (buffer & decision)" a few lines above it — only this one
   summary string is stale.
2. `src/game/terrain.ts`, doc comment on `canBearerEnterInnerCircle` (~line
   124): says "Rule 3 (an unladen Bearer already inside may only move onto
   G7 to collect the flag) is enforced in movement generation" — same stale
   claim, doesn't match `movement.ts` (which now allows free movement inside
   during the buffer/decision window).
3. `README.md` "Run them with `npm run test` (130 tests)" (line 180) — the
   actual current count is **148** across 13 files; the three newest test
   files (`opening-restriction.test.ts`, `starting-setup.test.ts`, and the
   growth of `board-render.test.ts`/others) aren't listed in the "### Tests"
   table either. `README.md` also has no mention of the Green opening
   restriction, the new starting setup, or the last-move indicator feature.
...
None of the above affect gameplay correctness — they are text-only
...
create `CLAUDE.md` and this file. If the user's next message doesn't specify
a task, ask what they'd like next rather than assuming. Natural candidates
if asked for a suggestion (not started, not requested):
- Fix the stale doc/comment/README inconsistencies listed above (small,
  docs/comments only, no rule change).
